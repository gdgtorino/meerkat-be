...
class SponsorService:
    ...
    def _doc_to_sponsor(self, sponsor_doc: DocumentReference) -> SponsorDto:
        ...
        if sponsor_doc.exists:
            out = SponsorDto(
                ...
                image=Image.model_validate(sponsor_doc.get("image"))
                if sponsor_doc.get("image")
                else None,
                btn=Btn.model_validate(sponsor_doc.get("btn"))
                if sponsor_doc.get("btn")
                else None,
            )

        return out

    def get_sponsor(self, id_sponsor: str) -> SponsorDto:
    ...
    def put_sponsor(self, sponsor: SponsorDto) -> SponsorDto:
        ...
        if sponsor_to_save.id:
            ...
            new_sponsor = self.db.collection("sponsor").add(
                sponsor_to_save.model_dump(
                    exclude={
                        "id",
                    }
                )
