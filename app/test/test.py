...
class TestDatabaseFirestore(unittest.TestCase):
    ...
    def test_get_firestore_client_with_prefix(self) -> None:
        from app.database.firestore import get_firestore_client

        # Configure the mock EnvYAML to return a prefix
        mock_env_instance = MagicMock()
        mock_env_instance.__getitem__.side_effect = (
            lambda key: "devfest_test" if key == "firestore.prefix" else None
        )
        self.mock_EnvYAML.return_value = mock_env_instance
    ...
    def test_get_firestore_client_without_prefix(self) -> None:
        from app.database.firestore import get_firestore_client

        # Configure the mock EnvYAML to return no prefix
        mock_env_instance = MagicMock()
        mock_env_instance.__getitem__.side_effect = (
            lambda key: None if key == "firestore.prefix" else None
        )
        self.mock_EnvYAML.return_value = mock_env_instance
...
class TestDtoHomePage(unittest.TestCase):
    def test_homepage_dto_valid_data(self) -> None:
        from app.dto.home_page import Header, Homepage, Menu
        from app.dto.page_elements import (
            Item,
        )
...
class TestDtoPageElements(unittest.TestCase):
    def test_btn_dto_valid_data(self) -> None:
    ...
    def test_image_dto_valid_data(self) -> None:
    ...
    def test_header_text_dto_valid_data(self) -> None:
    ...
    def test_background_dto_valid_data(self) -> None:
...
class TestModelsCommon(unittest.TestCase):
    def test_type_box_model_valid_data(self) -> None:
    ...
    def test_type_box_model_missing_name(self) -> None:
        from app.models.common.common import TypeBox
...
class TestModelsPage(unittest.TestCase):
    ...
    def test_item_model_valid_data(self) -> None:
        from app.models.common.common import TypeBox
...
class TestHomePageService(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.mock_firestore_client = patch(
            "app.services.homepage.get_firestore_client"
        ).start()
    ...
    def test_get_homepage_not_exists(self) -> None:
    ...
    def test_put_header(self) -> None:
        from app.dto.home_page import Header
        from app.dto.page_elements import Background, Btn, HeaderText, Image
        from app.services.homepage import HomePageService
    ...
    def test_put_section(self) -> None:
        from app.dto.page_elements import Item, TypeBox
        from app.services.homepage import HomePageService
...
class TestHomepageRouter(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.mock_homepage_service_class.return_value = (
            cls.mock_homepage_service_instance
        )
    ...
    def test_get_homepage_http_exception(self) -> None:
        from app.router.homepage import get_homepage

        self.mock_homepage_service_instance.get_homepage.side_effect = HTTPException(
    ...
    def test_get_homepage_generic_exception(self) -> None:
        from app.router.homepage import get_homepage

        self.mock_homepage_service_instance.get_homepage.side_effect = Exception(
        ...
        self.assertEqual(
            cm.exception.detail, "Qualcosa è andato storto riprova più tardi"
        )
        self.mock_homepage_service_instance.get_homepage.assert_called_once()
        self.mock_logging.error.assert_called_once()

    def test_put_header_success(self) -> None:
        from app.dto.home_page import Header
        ...
        header_data = Header(
            main_image=None,
            background_image=Background(is_image=False, color="#fff", image=None),
            header_text=None,
            call_to_action=[],
        )
        self.mock_homepage_service_instance.put_header.return_value = None

        response = put_header_route(
            header_data
